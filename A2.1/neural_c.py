...
with open(params, 'r') as file:
    data = file.read()
    layers = [int(i) for i in data[1:-1].split(',')]
...
def training(X, Y, model, epochs, lr, activation, lr_type, loss_type, batch_size, sgd, beta1, beta2):
...
epochs = training(X_train, Y_train, model, epochs,lr, activation, lr_type, 0, batch_size, 0, 0.9, 0.9)
...
my_params = [epochs, batch_size, lr_type, lr, activation, 1, 1]
