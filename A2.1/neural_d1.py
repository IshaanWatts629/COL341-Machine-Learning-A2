...
def training(X, Y, X_val, Y_val, model, epochs, lr, activation, lr_type, loss_type, batch_size, sgd, beta1, beta2):
    ...
    while time.time()-start < 270:
        ...
        Y_t = model.forward(X_val, activation, loss_type)
        lt = loss(Y_val, Y_t, loss_type)
        validation_loss.append(lt)
        i += 1

    predictions = model.predict(X_val, activation, loss_type)
    Y_val = np.argmax(Y_val, axis = 1)
    acc = np.sum(np.equal(predictions, Y_val).astype(float))/Y_val.shape[0]
    
    return training_loss, validation_loss, acc

layers = [[512, 256, 128, 64, 46], [256,46], [512, 256, 46], [256, 128, 46], [512, 256, 128, 46]]
batch = [64, 128, 256]
learning_rate = [0.001, 0.01, 0.1]
activation = [0, 1, 2]
lr_type = [0,1]

cols = ["SGD", "Batch Size", "Learning Rate", "Learning Rate Type", "Activation", "Accuracy", "Training Loss", "Validation Loss", "Epochs"]

def check(sgd_name, sgd, beta2, layer, layer_name):
        ...
        for lr_ in learning_rate:
            for b in batch:
                for ty in lr_type:
                    ...
                    if ty == 0:
                        d["Learning Rate Type"] = "Constant"
                    else:
                        d["Learning Rate Type"] = "Adaptive"

                    model = Neural_Network(input_layer = input_layer, layers = layer)
                    error_train, error_val, acc = training(X_train, Y_train, X_test, Y_test, model, 20, lr_, act, ty, 0, b, sgd, 0.9, beta2)
                    ...
                    path = sgd_name+'_'+layer_name+'_case_'+ str(case) + '.png'
                    plt.savefig(output_path+path)
                    plt.close()

                    d["Accuracy"] = acc
                    d["Training Loss"] = error_train[-1]
                    d["Validation Loss"] = error_val[-1]
                    d["Epochs"] = len(error_train)

                    table = table.append(d, ignore_index = True)
                    case += 1

    table.to_csv(output_path+'table_'+sgd_name+'_'+layer_name+'.csv')

check("Momentum", 0, 0.9, layers[0], "layer1")
check("Momentum", 0, 0.9, layers[1], "layer2")
check("Momentum", 0, 0.9, layers[2], "layer3")
check("Momentum", 0, 0.9, layers[3], "layer4")
check("Momentum", 0, 0.9, layers[4], "layer5")

check("Nesterov", 1, 0.9, layers[0], "layer1")
check("Nesterov", 1, 0.9, layers[1], "layer2")
check("Nesterov", 1, 0.9, layers[2], "layer3")
check("Nesterov", 1, 0.9, layers[3], "layer4")
check("Nesterov", 1, 0.9, layers[4], "layer5")

check("RMSprop", 2, 0.9, layers[0], "layer1")
check("RMSprop", 2, 0.9, layers[1], "layer2")
check("RMSprop", 2, 0.9, layers[2], "layer3")
check("RMSprop", 2, 0.9, layers[3], "layer4")
check("RMSprop", 2, 0.9, layers[4], "layer5")

check("Adam", 3, 0.999, layers[0], "layer1")
check("Adam", 3, 0.999, layers[1], "layer2")
check("Adam", 3, 0.999, layers[2], "layer3")
check("Adam", 3, 0.999, layers[3], "layer4")
check("Adam", 3, 0.999, layers[4], "layer5")

check("Nadam", 4, 0.999, layers[0], "layer1")
check("Nadam", 4, 0.999, layers[1], "layer2")
check("Nadam", 4, 0.999, layers[2], "layer3")
check("Nadam", 4, 0.999, layers[3], "layer4")
check("Nadam", 4, 0.999, layers[4], "layer5")
